...
/*18*/ "TYPE_CTYPES", // Count of TYPEs; Must be LAST + 1
];
...
function Work()
{
	...
	var pServerInfo = null;
	
	var cHandleEntries = 0;
	var wIndex = 0;
	...
	do
	{
		gSharedInfo = GetSymbolAddress('win32kbase!gSharedInfo');
		gpsi = GetSymbolAddress('win32kbase!gpsi');
		
		printf("gSharedInfo = 0x%p  \n" , gSharedInfo );
		printf("gpsi = 0x%p  \n" , gpsi );
		
		pServerInfo = ReadVirtualPointer(gpsi , 0x00);
		
		printf("pServerInfo = 0x%p  \n" , pServerInfo );
		
		cHandleEntries = ReadVirtualLong(pServerInfo ,  POINTER_SIZE );
		
		printf("cHandleEntries = 0x%x  \n" , cHandleEntries );
		
		aheList = ReadVirtualPointer(gSharedInfo , POINTER_SIZE * 1 );
		HeEntrySize = ReadVirtualLong(gSharedInfo , POINTER_SIZE * 2 );
		
		printf("aheList = 0x%p  \n" , aheList );
		printf("HeEntrySize = 0x%x  \n" , HeEntrySize );
		
		printf('\n');
		printf("|wIndex|wUniq|handle|phead|pOwner|ObjectType|bType|bFlags\n" );
		printf("|----|----|----|----|----|----|----|----\n" );
		
		for ( wIndex = 0; wIndex < cHandleEntries; wIndex++ )
		{
			pHeEntry =  aheList.add( HeEntrySize * wIndex );
			
			HandleEntry = {};
			
			HandleEntry.phead = ReadVirtualPointer( pHeEntry , POINTER_SIZE * 0 );
			HandleEntry.pOwner = ReadVirtualPointer( pHeEntry , POINTER_SIZE * 1 );
			
			HandleEntry.bType = ReadVirtualUChar( pHeEntry , POINTER_SIZE * 2 + 0x00 );
			
			if ( 255 == HandleEntry.bType )
			{
				strType = 'TYPE_GENERIC';
			}
			else if ( HandleEntry.bType < GUI_OBJECT_TYPE_TABLE.length )
			{
				strType = GUI_OBJECT_TYPE_TABLE[ HandleEntry.bType ];
			}
			else
			{
				strType = sprintf('TYPE_%d' , HandleEntry.bType );
			}
			
			
			HandleEntry.bFlags = ReadVirtualUChar( pHeEntry , POINTER_SIZE * 2 + 0x01 );
			HandleEntry.wUniq = ReadVirtualUShort( pHeEntry , POINTER_SIZE * 2  + 0x02 );
			
			printf("|0x%04x|0x%04x|0x%04x%04x|0x%p|0x%p|%s|%d|0x%02X\n" ,
					wIndex ,
					HandleEntry.wUniq ,
					HandleEntry.wUniq , wIndex ,
					HandleEntry.phead ,
					HandleEntry.pOwner ,
					strType ,
					HandleEntry.bType , 
